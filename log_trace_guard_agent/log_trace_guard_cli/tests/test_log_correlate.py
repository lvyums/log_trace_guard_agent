...
class TestChainAnalyzer(unittest.TestCase):
    ...
    def test_ssh_bruteforce_chain_detected(self):
        ...
        events = [
            CorrelatedEvent(timestamp="Jan 15 10:00:00", device_type="ssh", src_ip="10.0.0.1", status="failed",
                            raw_log="Jan 15 10:00:00 server sshd[123]: Failed password for root from 10.0.0.1 port 22 ssh2"),
            CorrelatedEvent(timestamp="Jan 15 10:00:01", device_type="ssh", src_ip="10.0.0.1", status="failed",
                            raw_log="Jan 15 10:00:01 server sshd[124]: Failed password for root from 10.0.0.1 port 22 ssh2"),
            CorrelatedEvent(timestamp="Jan 15 10:00:02", device_type="ssh", src_ip="10.0.0.1", status="failed",
                            raw_log="Jan 15 10:00:02 server sshd[125]: Failed password for root from 10.0.0.1 port 22 ssh2"),
            CorrelatedEvent(timestamp="Jan 15 10:00:03", device_type="ssh", src_ip="10.0.0.1", status="failed",
                            raw_log="Jan 15 10:00:03 server sshd[126]: Failed password for root from 10.0.0.1 port 22 ssh2"),
            CorrelatedEvent(timestamp="Jan 15 10:00:05", device_type="ssh", src_ip="10.0.0.1", status="success",
                            raw_log="Jan 15 10:00:05 server sshd[127]: Accepted password for root from 10.0.0.1 port 22 ssh2"),
        ]
        ...
        ssh_chain = [c for c in chains if "ssh_brute" in c.chain_id or "brute_force" in c.chain_id]
    ...
    def test_web_attack_db_chain(self):
        ...
        events = [
            CorrelatedEvent(timestamp="Jan 15 10:00:00", device_type="waf", src_ip="10.0.0.7", status="blocked",
                            ...
                            raw_log='Jan 15 10:00:00 server WAF: blocked SQL injection from 10.0.0.7 severity=high'),
            CorrelatedEvent(timestamp="Jan 15 10:00:01", device_type="web", src_ip="10.0.0.7", status="500",
                            raw_log='Jan 15 10:00:01 server 10.0.0.7 - - [15/Jan/2024:10:00:01] "GET /admin?id=1 UNION SELECT * FROM users HTTP/1.1" 500 1234'),
            CorrelatedEvent(timestamp="Jan 15 10:00:02", device_type="db", src_ip="10.0.0.7", status="error",
                            raw_log='Jan 15 10:00:02 server mysql: 2024-01-15T10:00:02 user=root query=SELECT * FROM users'),
        ]
        groups = {"10.0.0.7": events}
        chains = self.analyzer.analyze(events, groups, TimelineBuilder(time_window_minutes=10).get_time_window())
        web_chains = [c for c in chains if "sql_injection" in c.chain_id or "web_scan" in c.chain_id or "waf_alert" in c.chain_id]
    ...
    def test_no_false_positive_for_single_event(self):
        ...
        ssh_chain = [c for c in chains if "ssh_brute" in c.chain_id]
        self.assertEqual(len(ssh_chain), 0)
        # Single event should not produce ANY chain with confidence > 0.3
        high_conf = [c for c in chains if c.confidence > 0.3]
...
class TestIntegration(unittest.TestCase):
    def test_full_pipeline_ssh_chain(self):
        ...
        self.assertGreaterEqual(len(chains), 1)
        first = chains[0]
        self.assertIn("ssh_brute", first.chain_name.lower())
