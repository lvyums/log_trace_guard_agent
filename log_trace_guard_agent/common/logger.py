...
class LogManager:
    ...
    def __init__(self):
        ...
        self._logger = logging.getLogger("LogTraceGuard")
    ...
    @classmethod
    def log_parse_failure(cls, raw_log: str, reason: str):
        """日志解析失败时留存原始日志片段"""
        snippet = raw_log[:200] if raw_log else ""
        cls.get_logger().warning(f"[PARSE_FAIL] {reason} | raw={snippet}")