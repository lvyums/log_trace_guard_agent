<template>
  <div class="g-stack">
    <AlertGuide type="warning" title="风险研判需要结合上下文">
      单条日志的风险判断有局限性。建议先通过「日志识别」确认设备类型，再到「结构化解析」提取关键字段，最后做综合研判。
    </AlertGuide>
    ...
    <div class="g-card">
      <div class="g-card-header">
        <div>
          ...
          <div class="g-card-desc">对一条日志内容进行安全风险评估，标注风险等级（P0-P3）与处置建议</div>
        </div>
        <el-button size="small" type="primary" plain @click="fillSample">填充测试日志</el-button>
      </div>
      <el-input
        v-model="input" type="textarea" :rows="4"
        placeholder="粘贴一条待研判的日志..." :disabled="loading"
        @keyup.ctrl.enter="submit"
      />
      <div class="g-input-guide">
        <el-icon><InfoFilled /></el-icon>
        <span>仅支持单条日志研判。可指定设备类型（SSH/Web/WAF/防火墙/数据库）提高精度。</span>
      </div>
