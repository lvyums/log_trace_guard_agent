...
AI 驱动的日志分析与安全威胁狩猎平台，提供日志解析、合规审计、日志采集、脚本生成、规划咨询、安全威胁狩猎、攻防实训七大模块。
...
```bash
cd log_trace_guard_agent/log_trace_guard_cli
...
CLI 提供日志解析、日志采集、脚本生成、合规审计、日志联合审查、AI 智能对话六大功能，支持离线场景。
...
完整配置参见 `.env.example`。系统设置面板（前端右上角齿轮图标）支持在线配置 API Key 和 Splunk 连接信息。
...
| 日志采集       | `/api/v1/log-collect`           | 设备匹配、采集方案、架构推荐、故障诊断 |
| 脚本生成       | `/api/v1/script-gen`            | 正则生成、ES 查询、攻击溯源、脚本优化 |
| 合规审计       | `/api/v1/compliance`            | 等保基线生成、合规校验           |
| **规划咨询**   | `/api/v1/advisory`              | 采集方案、架构推荐、平台选型、指导手册（v3.1 新增） |
| **安全威胁狩猎** | `/api/v1/log-correlate`       | 多源日志攻击链检测（v3.0 新增）  |
| 实训交互       | `/api/v1/training`              | 攻防场景、任务执行、报告生成     |

## 导航模块

前端 SPA 包含 **7 个导航模块、22 个子页面**：
