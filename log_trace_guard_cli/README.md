...
联合日志审查子菜单扩展为 **4 项功能**，新增「攻击链 → 溯源脚本」闭环：

1. **关联分析** — 关键词 + LLM 双引擎，检测 **16 种攻击链**（SSH暴力破解后提权 / SQL注入 / 数据窃取 / C2通信 / 勒索软件等）
2. **大文件分片分析** — 超大日志分片处理
3. **攻击链列表** — 查看全部可检测攻击链模式
4. **攻击链 → 溯源脚本** — 检出攻击链后一键生成溯源脚本，复用既有闭环：

```
关联分析 → 检出攻击链（16种） → 选择攻击链 → 自动推断溯源类型
  → 生成溯源脚本 → 可选：导出报告（Markdown/JSON）+ 生成监控规则（ES DSL + 正则）
```

> **与脚本生成的区别**：脚本生成「攻击溯源」是手动输入攻击类型；这里是**从关联分析结果直接联动**，分析出什么攻击链就溯源什么，无需重复描述。

攻击链 → 溯源类型自动映射（16 链名 → 7 种 trace 类型）：

| 攻击链示例 | 溯源类型 |
|------|------|
| SSH暴力破解后提权 | brute_force |
| SQL注入攻击链 | sql_injection |
| 数据窃取攻击链 | data_exfil |
| Web攻击到数据窃取 | web_attack |
| 权限提升攻击链 | privilege_escalation |
| 内网侦察攻击链 | recon |
| 其他 / 未识别 | brute_force（默认回退） |
